...
def TrainModel():
    ...
    for traincv, testcv in kf.split(tweetList):
        ...
        anova_svm = Pipeline([('anova', anova_filter), ('svc', classifier)])

        anova_svm.set_params(anova__k=1000, svc__C=.1).fit(X_train_tfidf, xLabelList)
        ...
        predicted = anova_svm.predict(X_new_tfidf)
